...
function [D]=diff_coef_soft(n,incident_angle,diffracted_angle,L)
...
dummy1=sin(psi_0)-sqrt(epsilon_com-cos(psi_0).^2);
dummy2=sin(psi_0)+sqrt(epsilon_com-cos(psi_0).^2);
r_0_soft=(dummy1./dummy2);

dummy3=sin(psi_n)-sqrt(epsilon_com-cos(psi_n).^2);
dummy4=sin(psi_n)+sqrt(epsilon_com-cos(psi_n).^2);
r_n_soft=(dummy3./dummy4);

D=(r_0_soft.*r_n_soft.*D1)+D2+(r_0_soft.*D3)+(r_n_soft.*D4);
